...
const updateWeatherUI = (data) => {
    if (data && data.weather && data.weather.length > 0) {
        ...
        document.querySelector('.weather-details .detail-item:nth-child(4) p').textContent = `UV Index: 8`; // UV Index not available in this endpoint
        ...
        getForecastData(data.name).then(forecastData => {
            if (forecastData && forecastData.list) {
                ...
                dailyForecast.innerHTML = forecastData.list
                    .filter(item => item.dt_txt.endsWith("12:00:00")) // Get forecast data for 12:00 PM only
                ...
                hourlyForecast.innerHTML = forecastData.list
                    .filter(item => item.dt_txt.endsWith("00:00:00") || item.dt_txt.endsWith("03:00:00") || item.dt_txt.endsWith("06:00:00") || item.dt_txt.endsWith("09:00:00") || item.dt_txt.endsWith("12:00:00") || item.dt_txt.endsWith("15:00:00") || item.dt_txt.endsWith("18:00:00") || item.dt_txt.endsWith("21:00:00")) // Get forecast data for specific hours
